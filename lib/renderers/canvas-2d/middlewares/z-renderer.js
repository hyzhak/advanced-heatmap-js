...
export default class ZRenderer {
  ...
  _validateFeatureSchemeOfValues (features) {
    if (features.alpha) {
      ...
      features.alpha = this._validateValuesMinMax();
    }
    ...
    if (features.radius) {
      ...
      features.radius = this._validateValuesMinMax(null, { max: 16 });
    }
  ...
  async process (ctx) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const radius = Math.round(this._extractFeature(featuresScheme.radius, data[i]));
      const template = this._getTemplate(radius);
      shadowCanvasCtx.globalAlpha = this._extractFeature(featuresScheme.alpha, data[i]);
